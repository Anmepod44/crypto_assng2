...
def generate_rsa_keys(key_size, key_name):
    ...
    with open(os.path.join(base_directory, f'outputs/{key_name}_private.pem'), 'wb') as priv_file:
        priv_file.write(private_key.export_key())

    # Save the public key to a file
    with open(os.path.join(base_directory, f'outputs/{key_name}_public.pem'), 'wb') as pub_file:
...
def load_rsa_keys(key_name):
    with open(os.path.join(base_directory, f'outputs/{key_name}_private.pem'), 'rb') as priv_file:
        private_key = RSA.import_key(priv_file.read())

    with open(os.path.join(base_directory, f'outputs/{key_name}_public.pem'), 'rb') as pub_file:
